...
class LearnedGradientDescentOptimizer(nn.Module, OptimizerBase):
    ...
    def perform_step_without_gradlikelihood(self, latent_images: th.Tensor, degraded_images: th.Tensor,
                                            hidden_state: th.Tensor = None) -> th.Tensor:
        ...
        if self.hidden_state_backbone is not None:
            assert hidden_state is not None
            hidden_state = self.hidden_state_backbone(th.cat((latent_images, hidden_state), dim=1))
            network_input = th.cat((latent_images, hidden_state), dim=1)
        else:
    ...
    def restore_without_gradlikelihood(self, degraded_images: th.Tensor, track_updates_history: bool = False
                                       ) -> Union[List[th.Tensor], th.Tensor]:
        # TODO: change docstring
        """
        This method performs restoration of input degraded images.
        ...
        if self.hidden_state_backbone is not None:
            ...
            hidden_state = None
        for i in range(self.num_steps):
            latent_images, hidden_state = self.perform_step_without_gradlikelihood(latent_images, degraded_images,
                                                                                   hidden_state)
            latent_images = self.project(latent_images)
            if track_updates_history:
                history.append(latent_images)
    ...
    def forward(self, degraded_images: th.Tensor) -> List[th.Tensor]:
        ...
        """
        return self.restore_without_gradlikelihood(degraded_images, track_updates_history=True)
